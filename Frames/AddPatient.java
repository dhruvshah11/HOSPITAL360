...
public class AddPatient extends JFrame implements MouseListener, ActionListener{
	...
	public AddPatient(PList pl){
		...
		f1 = new Font("Ubuntu", Font.BOLD, 30);
		f2 = new Font("Ubuntu", Font.BOLD, 18);
		...
		String command = ae.getActionCommand();
		    if (addBtn.getText().equals(command)){
				try {
					...
					String PName = pnTF.getText().trim();
					String Mobile = mbTF.getText().trim();
					String age = ageTF.getText().trim();
					String address = addTF.getText().trim(); 
					String Diagnosis = disTF.getText().trim();
					String deposit = depTF.getText().trim();
					...
					String appDoctor = b1.getSelectedItem().toString();
					String room = b2.getSelectedItem().toString();

					// Validate all fields
					if(PName.isEmpty() || Mobile.isEmpty() || gender.isEmpty() || age.isEmpty() || 
					   address.isEmpty() || appDoctor.isEmpty() || room.isEmpty() || 
					   Diagnosis.isEmpty() || deposit.isEmpty() || room.equals(" ")){
						JOptionPane.showMessageDialog(this, "Please fill in all fields and select a room!");
						return;
					}
					...
					try {
						...
						if(ageNum <= 0 || ageNum > 150) {
							JOptionPane.showMessageDialog(this, "Please enter a valid age between 1 and 150!");
							return;
						}
